...
results = []
for drop, lamda, lr in itertools.product(drops, lamdas, lrs):
    args.lamda = lamda
    args.lr = lr
    args.drop = drop
    acc_te_i = []
    ...
    for i in range(args.repeat):
        ...
        for j in range(args.epochs):
            ...
            if (j + 1) % log_step == 0:
                ...
                for key, result in temp_results.items():
                    ...
                    msg += (key + f'{{v:{valid_res:.4f}, te:{test_res:.4f}}} ')
                print('epoch :{:3d}, loss:{:.1f}, loss_v:{:.4f}, loss_t:{:.4f}'
            ...
            # model.scheduler.step(temp_val['acc'])
        ...
        if args.save:
            ...
            emb_in_l = model.get_node_embedding()
            path = osp.join(dataset.root, 'processed/MF_emb.pt')
            joblib.dump(emb_in_l.numpy(), path, compress=3)
        del model
        gc.collect()
